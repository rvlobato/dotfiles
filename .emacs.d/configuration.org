...
(use-package company-wordfreq
  :config
  ;; 1. The Eglot Gatekeeper
  (defun my-eglot-latex-gate (orig-fn &rest args)
    "Prevent Eglot from hijacking normal text in LaTeX."
    (if (and (derived-mode-p 'latex-mode 'TeX-mode 'LaTeX-mode)
	     ;; Look at the start of the current word
	     (let ((start-pos (save-excursion (skip-syntax-backward "w_") (point))))
	       ;; If the character right before the word is NOT \, {, or [
	       (not (memq (char-before start-pos) '(?\\ ?{ ?[)))))
	;; Then return nil (force Eglot to step aside for English text)
	nil
      ;; Otherwise, run Eglot normally for LaTeX code
      (apply orig-fn args)))

  ;; Attach the gatekeeper directly to Eglot
  (advice-add 'eglot-completion-at-point :around #'my-eglot-latex-gate)

  ;; 2. The Cleaned Up Backend List
  (defun my-apply-writing-backends ()
    (setq-local company-backends
		'(
		  ;; Layer 1: Eglot (Safely controlled by the gatekeeper above)
		  company-capf

		  ;; Layer 2: Text Dictionaries
		  ;; Grouped together so dabbrev, wordfreq, and ispell merge into one list
		  (company-dabbrev :with company-wordfreq company-ispell)

		  ;; Layer 3: Math and Files
		  company-math-symbols-latex
		  company-files
		  ))
    (setq-local company-transformers nil))

  ;; 3. Apply to LaTeX modes AFTER Eglot finishes loading
  (add-hook 'eglot-managed-mode-hook
	    (lambda ()
	      (when (derived-mode-p 'latex-mode 'TeX-mode 'LaTeX-mode)
		(my-apply-writing-backends))))

  ;; 4. Apply to standard text modes
...
(use-package eglot
  ...
  (add-hook 'eglot-managed-mode-hook
	    (lambda ()
	      ;; Check if we are in a text/TeX related mode where LanguageTool makes sense
	      (when (derived-mode-p 'tex-mode 'latex-mode 'text-mode 'org-mode 'markdown-mode)
